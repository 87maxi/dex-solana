...
export function WalletConnectModal() {
  ...
  const handleSolanaConnect = async (walletName: any) => {
    try {
      ...
      select(walletName);
      // Wait for a brief moment to ensure the wallet is selected
      await new Promise((resolve) => setTimeout(resolve, 500));
      // Check if a wallet is connected before proceeding
      if (!solanaConnected) {
        await solanaConnect();
      }
      setOpen(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-md bg-white border-slate-200">
        ...
        <div className="space-y-6 py-4">
          ...
          <div>
            <h3 className="text-xs font-bold uppercase tracking-wider text-indigo-500 mb-3">
              EVM Wallets
            </h3>
            <div className="grid grid-cols-1 gap-2">
