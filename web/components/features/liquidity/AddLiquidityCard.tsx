...
export function AddLiquidityCard(): React.ReactNode {
    ...
    const [estimatedLPTokens, setEstimatedLPTokens] = useState<bigint | undefined>();
    const [busy, setBusy] = useState(false);
    const [status, setStatus] = useState("");
    const [isTokenABalanceLoading, setIsTokenABalanceLoading] = useState(false);
    const [isTokenBBalanceLoading, setIsTokenBBalanceLoading] = useState(false);

    const parsedAmountA = amountA
        ? BigInt(Math.round(parseFloat(amountA) * 1e6))
        : 0n; // 6 decimals

    const parsedAmountB = amountB
        ? BigInt(Math.round(parseFloat(amountB) * 1e6))
        : 0n; // 6 decimals;
    ...
    useEffect(() => {
        ...
        // Mock balances - en implementación real se buscarían los balances reales
        setTokenABalance(1000000000000n); // 1000 TA
        setTokenBBalance(2000000000000n); // 2000 TB
    }, [isConnected, address]);

    useEffect(() => {
        if (parsedAmountA > 0n && parsedAmountB > 0n) {
            // Mock estimation - en implementación real se calcularía en el contrato
            setEstimatedLPTokens(parsedAmountA + parsedAmountB);
        } else {
            setEstimatedLPTokens(undefined);
        }
    ...
    async function handleAddLiquidity(): Promise<void> {
        try {
            ...
            if (chainType === "solana") {
                ...
                txHash = await DEXService.solanaAddLiquidity(
                    wallet.adapter,
                    connection,
                    parsedAmountA,
                    parsedAmountB
            ...
            setEstimatedLPTokens(undefined);
            setStatus("");
            alert(`Liquidity added successfully! Transaction: ${txHash}`);
        } catch (err) {
            ...
            <div className="space-y-4">
                <div className="group">
                    <div className="flex items-center justify-between mb-2 px-1">
                        ...
                        <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">
                            Balance: <span className="font-mono text-slate-600">{fmt(tokenABalance)}</span>
                        </span>
                    </div>
                    <div className="relative group/input">
                        ...
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-100 shadow-sm">
                            ...
                            <span className="text-sm font-bold text-slate-700">TA</span>
                        </div>
                    </div>
                </div>

                <div className="flex justify-center -my-2 relative z-10">
                    <div className="flex h-10 w-10 items-center justify-center rounded-2xl border-4 border-white bg-teal text-white shadow-lg shadow-teal/20">
                        <svg
                            className="h-5 w-5"
                            fill="none"
                            viewBox="0 0 24 24"
                ...
                <div>
                    <div className="flex items-center justify-between mb-2 px-1">
                        ...
                        <span className="text-xs font-medium text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">
                            Balance: <span className="font-mono text-slate-600">{fmt(tokenBBalance)}</span>
                        </span>
                    </div>
                    <div className="relative group/input">
                        <Input
                        ...
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 bg-white px-3 py-1.5 rounded-xl border border-slate-100 shadow-sm">
                            ...
                            <span className="text-sm font-bold text-slate-700">TB</span>
                        </div>
                    </div>
                </div>

                {estimatedLPTokens !== undefined && (
